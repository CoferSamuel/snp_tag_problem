...
Como aclaración inicial, debe señalarse la ausencia del algoritmo MOEA/D en los resultados. Aunque este algoritmo basado en descomposición fue contemplado en el diseño experimental, la implementación interna de la librería \texttt{pymoo} \cite{pymoo} carece de soporte nativo para el manejo de restricciones durante la ejecución de su versión base. Si bien existen variantes en la literatura científica que dotan a MOEA/D de mecanismos para gestionar este tipo de espacios \cite{jan2010moea}, la implementación oficial del marco de optimización utilizado resulta incompatible con la inecuación matemática ($G(X) \le 0$) impuesta para garantizar la factibilidad biológica del problema de selección de etiquetas SNP \cite{bafna2003haplotypes}. Por consiguiente, este algoritmo debió ser excluido de las simulaciones.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_nsga2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_spea2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_nsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_unsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_rvea_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_smsemoa_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_agemoea2_full_31.png}
...
El análisis de los frentes de Pareto agregados revela una jerarquía en el impacto de los distintos componentes sobre el espacio de soluciones del problema de selección de etiquetas SNP.

En primer lugar, se observa que las estrategias de inicialización poblacional determinan la región del espacio objetivo hacia la que converge cada frente de manera predominante, evidenciando una correlación directa entre los cuatro objetivos matemáticos evaluados. La estrategia \textit{greedy\_multi} concentra las soluciones en la zona de máxima compacidad (generando soluciones con un número reducido de ~30 SNP de media), aunque a costa de minimizar simultáneamente los niveles del resto de objetivos (de media): la tolerancia cae a ~6 fallos, la disimilitud a ~15 y el balance a ~10. Esta configuración restringe la diversidad de la búsqueda y aproxima a los individuos al límite estricto de la factibilidad biológica ($f_2 = 1$).

Por el contrario, la inicialización \textit{random\_dense} orienta la búsqueda hacia el extremo opuesto del hiperespacio, hallando soluciones con valores medios muy elevados en tolerancia (~73), disimilitud (~133) y balance (~446, valor subóptimo), aumentando también la compacidad (requiriendo la selección de una cantidad excesiva de ~345 SNP). Finalmente, la estrategia \textit{greedy\_ting} actúa como un punto de compromiso, compensando el rendimiento conjunto en los cuatro objetivos matemáticos (compacidad de ~232 SNP, tolerancia de ~44, disimilitud de ~89 y balance de ~260) y permitiendo explorar la zona central del frente de Pareto.

En segundo lugar, el rendimiento algorítmico presenta variaciones según el paradigma utilizado. Los EAs basados en dominancia de Pareto (NSGA-II y SPEA2) y en geometría de proximidad (AGE-MOEA-II) muestran una mayor capacidad de retención de individuos no dominados, generando los frentes con la cardinalidad más alta del estudio (~220 soluciones válidas de media por ejecución). De igual modo, SMS-EMOA mantiene un nivel de densidad poblacional altamente competitivo en base a su optimización de hipervolumen, promediando ~196 soluciones.

En contraste, los MaOEA (NSGA-III, U-NSGA-III y RVEA), basados en la descomposición del espacio mediante vectores de referencia, evidencian limitaciones en este escenario: al proyectar dichas direcciones de forma uniforme sobre un espacio de búsqueda fuertemente sesgado por la inecuación de factibilidad, muchas de esas líneas de referencia no logran asociarse a soluciones válidas. Como resultado, estos algoritmos sufren una pérdida de individuos y producen frentes con una densidad poblacional marcadamente inferior (promediando ~26 soluciones en NSGA-III, ~25 en U-NSGA-III y cayendo hasta el mínimo de ~24 soluciones válidas en RVEA).

Finalmente, el análisis cuantitativo de los frentes confirma la ausencia de diferencias significativas atribuibles a los operadores de cruce. Al evaluar los valores medios globales de las soluciones generadas por cada variante, los resultados resultan similares en los cuatro objetivos: los cruces de un punto y dos puntos alcanzan una compacidad promedio de ~201 y ~202 SNP, con tolerancias de ~39 y ~40, disimilitudes de ~77 y ~78, y balances de ~238 y ~243, respectivamente.

De forma análoga, el cruce uniforme y el medio uniforme convergen en compacidades medias de ~205 y ~201 SNP, con tolerancias de ~43, disimilitudes de ~81 y ~80, y balances de ~237 y ~238, respectivamente. Estas cifras constatan que las distribuciones de los cuatro operadores se encuentran superpuestas en el espacio objetivo, lo que indica que, para el problema de selección de etiquetas SNP, el mecanismo de recombinación de material genético queda relegado a un segundo plano por el profundo sesgo espacial que impone la inicialización poblacional.

\subsection{Dinámica de convergencia}

El análisis de la dinámica de convergencia permite observar cómo evoluciona la calidad geométrica de la población a lo largo del proceso de optimización. A continuación, se presentan las trayectorias generacionales del hipervolumen para cada algoritmo, extraídas utilizando el filtrado estadístico por réplica mediana detallado en la sección metodológica.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_nsga2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_spea2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_nsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_unsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_rvea_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_smsemoa_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/2_metricas_convergencia/convergencia_hv_agemoea2_full_31.png}
...
Por otro lado, el comportamiento de RVEA constituye la única divergencia estructural apreciable en estos gráficos del experimento. Con la inicialización \textit{Greedy Ting}, su hipervolumen experimenta un decrecimiento general: las configuraciones pierden de media en torno a un $40$\% respecto a su estado inicial. A diferencia de otros algoritmos basados en vectores, RVEA evalúa a los individuos utilizando la Distancia Penalizada por Ángulo (APD). Como detallan los propios creadores del algoritmo (Cheng et al.), la arquitectura de RVEA asume que no es necesario normalizar el espacio de objetivos al rango $[0, 1]$ porque la adaptación de los vectores de referencia soluciona las diferencias de escala~\cite{cheng2016reference}.

Sin embargo, aunque esta adaptación corrige el cálculo de los ángulos, la componente base de la APD sigue siendo la distancia euclidiana bruta al punto ideal. Dado que el espacio objetivo de este problema presenta escalas muy distintas —con el Balance ($f_4 \in [0, 2140]$) y la Compacidad ($f_1 \in [1, 772]$) superando a la Disimilitud ($f_3 \in [0, 256]$) y a la Tolerancia ($f_2 \in [0, 129]$)—, la asunción teórica de Cheng resulta subóptima para este problema: la distancia euclidiana bruta (sin escalar) queda dominada por las magnitudes mayores.

Para evitar la penalización de la métrica APD, el algoritmo se ve forzado a reducir el Balance y la Compacidad. La experimentación confirma este fallo estructural: al promediar las 31 réplicas generadas, el $93.2\%$ de la población de RVEA se compone de paneles reducidos (por debajo de las 70 etiquetas SNP). Al concentrarse en el origen del espacio objetivo para minimizar las distancias brutas de $f_4$ y $f_1$, RVEA ignora por completo la exploración del resto del frente de Pareto y sufre pérdida de diversidad.

Esta limitación provoca otro problema: al concentrarse hacia el origen (buscando paneles de SNPs reducidos), el algoritmo cruza la frontera de factibilidad biológica del problema. Los paneles pequeños son incapaces de distinguir a todos los pares de haplotipos del conjunto de datos, lo que provoca que su tolerancia caiga a cero ($f_2 = 0$) e infrinjan la restricción matemática del problema ($G(X)$).

Al finalizar las ejecuciones, una media de $202.7$ individuos de la población final de RVEA (el $92\%$) son soluciones biológicamente infactibles. Mientras que NSGA-II o SMS-EMOA logran sostener frentes compuestos por $220$ soluciones válidas, RVEA tiene una alta población con soluciones infactibles. Al aplicar el filtro de factibilidad para extraer el frente de Pareto definitivo, RVEA experimenta un filtrado que reduce su cardinalidad útil a una media general de apenas $24$ soluciones.

\subsubsection{Análisis de las inicializaciones}

Las estrategias de inicialización establecen un punto de partida cuantificable desde la generación cero. Tanto \textit{Greedy Ting} como \textit{Random Dense} inician con valores medios de hipervolumen en torno a $0{,}255$, mientras que \textit{Greedy Multi} parte de $\approx 0{,}023$ en todos los algoritmos. Esta diferencia de magnitud se mantiene a lo largo de la evolución: tras 500 generaciones, las configuraciones con \textit{Greedy Multi} apenas alcanzan una media de $0{,}045$, mientras que las otras dos inicializaciones superan los $0{,}40$ en los algoritmos con mayor hipervolumen final. Se aprecia un efecto catapulta en todos los algoritmos (salvo RVEA) con las inicializaciones \textit{Greedy Ting} y \textit{Random Dense}, ya que ambas proveen a la población inicial de un conjunto de soluciones que elevan el hipervolumen en las primeras etapas, ahorrando cientos de generaciones de exploración básica frente al arranque de \textit{Greedy Multi}.
...
Para adaptar el volumen de datos a las dimensiones del documento (véase la Tabla \ref{tab:resultados_metricas}), se ha establecido un sistema de abreviaturas para identificar cada configuración y métrica. Las configuraciones algorítmicas se estructuran bajo la nomenclatura \textit{Algoritmo-Inicialización-Cruce}. Específicamente, los algoritmos se denotan como NS2 (NSGA-II), SP2 (SPEA2), SMS (SMS-EMOA), NS3 (NSGA-III), US3 (U-NSGA-III), RV (RVEA) y AG2 (AGE-MOEA-II).

Las estrategias de inicialización se abrevian como GM (\textit{Greedy Multi}), GT (\textit{Greedy Ting}) y RD (\textit{Random Dense}), mientras que los operadores de recombinación se indican mediante 1P (un punto), 2P (dos puntos), UX (uniforme) y HUX (medio uniforme). De forma análoga, para las métricas se utilizan los acrónimos HV (Hipervolumen), IGD+ (Error de generación invertido plus), GD+ (Error de generación plus), Rng (Rango), MSum (MínSuma), SMin (SumaMín), Tmed (Tolerancia media), Tmáx (Tolerancia máxima) y Hmed (Distancia media de Hamming).

\begingroup
\tiny
\setlength{\tabcolsep}{0.5pt}
\begin{longtable}{@{}lccccccccc@{}}
\caption[Resultados de las métricas de evaluación (Absoluto)]{Resultados globales del experimento absoluto (valores promedio y desviación típica en el subíndice a lo largo de 31 ejecuciones independientes). Se resalta en negrita el \textit{Top 5} de configuraciones algorítmicas con mejor rendimiento para cada métrica individual.}
\label{tab:resultados_metricas} \\
\toprule
\textbf{Config} & \textbf{HV} & \textbf{IGD+} & \textbf{GD+} & \textbf{Rng} & \textbf{MSum} & \textbf{SMin} & \textbf{Tmed} & \textbf{Tmáx} & \textbf{Hmed} \\
\midrule
\endfirsthead
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_hypervolume_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hipervolumen (Absoluto): Algoritmos}
    \label{fig:dunn_hv_abs_alg}
\end{figure}

Desde la perspectiva de los EAs aislados, el test de significancia permite estructurar el rendimiento en niveles diferenciados. Resulta destacable que NSGA-II exhiba el mayor rendimiento a pesar de estar diseñado originalmente para MOP. Al escalar a cuatro objetivos, se produce una pérdida de presión por el mecanismo de ordenación de frentes: el análisis de las trazas poblacionales confirma que, de media, la dominancia de Pareto de NSGA-II se satura por completo en la generación 36, momento en el cual el 100\% absoluto de la población pasa a residir en el primer frente.

Como consecuencia, la distancia de agrupamiento asume el protagonismo total de la selección, actuando como estimador de diversidad en el espacio de objetivos. Si bien los estudios clásicos advierten que este operador pierde eficacia en MaOPs continuos ~\cite{purshouse2003evolutionary, ishibuchi2008evolutionary, deb2013evolutionary}, Ishibuchi et al.~\cite{ishibuchi2015behavior} demuestran que NSGA-II puede superar a otros algoritmos cuando los objetivos presentan una alta correlación entre sí. 

Este trabajo aporta evidencia empírica de que dicha condición se cumple (Figura~\ref{fig:corr_spearman_abs}): el análisis de correlación de Spearman aplicado sobre el conjunto completo de soluciones del frente de Pareto final ($n = 347\,545$) revela que todos los pares de objetivos presentan una correlación positiva y estadísticamente significativa ($\rho \geq 0{,}96$, $p < 0{,}001$). Esta formulación altamente correlacionada del espacio objetivo hace que la distancia de agrupamiento, en ausencia de presión de dominancia efectiva, actúe como un estimador de diversidad global eficaz, promoviendo una expansión del frente que maximiza el hipervolumen.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../exp_aux/correlacion/corr_spearman_experimento_abs.png}
    \caption[Análisis de correlación de Spearman (Absoluto)]{Matriz de correlación de Spearman (izquierda) y p-valores en escala logarítmica (derecha) para los cuatro objetivos en el experimento absoluto. El análisis estadístico confirma una sinergia casi perfecta ($\rho \geq 0{,}96$, $p < 0{,}001$) entre todas las métricas, lo que reduce drásticamente la dimensionalidad efectiva del espacio de búsqueda.}
    \label{fig:corr_spearman_abs}
\end{figure}
...
Por su parte, SMS-EMOA ocupa el penúltimo estrato de rendimiento de la clasificación. Su bajo desempeño en la métrica de hipervolumen es una consecuencia directa de un estancamiento prematuro derivado de una pérdida de diversidad genética. SMS-EMOA concentra al 100\% de su población en el primer frente de Pareto en una media de apenas 18,5 generaciones, operando el 96\% restante de la ejecución bajo un estancamiento relevante.

El estancamiento se produce por la estrategia de evolución de estado estacionario ($\mu+1$) operando sobre un espacio combinatorio altamente discreto. Como detallan Beume et al. en la propuesta original de SMS-EMOA, la arquitectura $\mu+1$ se adoptó para reducir el alto coste computacional que exige el cálculo del hipervolumen, admitiendo la carencia de operadores de variación especializados para gestionar la diversidad~\cite{beume2007sms}.

Al generar un único individuo por iteración, la diversidad que se genera es muy escasa. Esta reducción de la diversidad poblacional afecta negativamente en ausencia de un mecanismo explícito de preservación de nichos~\cite{deb2001multi} y depender exclusivamente en la contribución al hipervolumen, como se demuestra, no ofrece buenos resultados.

Con respecto a la ausencia de mecanismos de preservación de nichos, la literatura reciente ya ha documentado este fallo de SMS-EMOA en problemas combinatorios y discretos: el análisis de Ishibuchi et al.~\cite{ishibuchi2015behavior} demuestra que permitir a algoritmos como SMS-EMOA elegir progenitores de toda la población de forma global empeora la diversidad.

Su estudio concluye que, debido a su estructura espacial, la selección desplaza el frente hacia regiones centrales estancadas; para evitar que algoritmos basados en hipervolumen se estanquen en estos espacios discretos, es necesario recombinar soluciones locales muy similares (vecindarios), protegiendo la diversidad frente a la convergencia central. El análisis de las ejecuciones confirma este fenómeno limitante: independientemente del operador de cruce utilizado, el grueso de la población de SMS-EMOA se concentra permanentemente en un estrato central estrecho, perdiendo el material genético necesario para expandirse hacia los extremos.

Finalmente, RVEA se posiciona en el último estrato estadístico de forma significativa. En este caso, la deficiencia de la métrica APD (que asume una homogeneidad de escalas inexistente en este problema) fuerza a la población a converger hacia el origen del espacio objetivo. Este desplazamiento provoca que el algoritmo cruce la frontera de factibilidad biológica; al finalizar las ejecuciones, el $92\%$ de sus individuos representa soluciones inválidas (con tolerancia cero), reduciendo su cardinalidad útil a una media de apenas 24 soluciones factibles y relegando al EA a la última posición absoluta.

\subparagraph{Análisis por inicialización}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_hypervolume_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hipervolumen (Absoluto): Inicialización}
    \label{fig:dunn_hv_abs_init}
\end{figure}

Al evaluar las estrategias de inicialización de forma aislada, el test estadístico confirma los patrones identificados en la dinámica de convergencia. La inicialización \textit{Random Dense} se sitúa en el primer nivel de significancia al inyectar la máxima diversidad estocástica desde la generación cero. En el segundo escalón se posiciona \textit{Greedy Ting}, la cual logra un rendimiento destacado debido a su efecto de impulso inicial, aunque con menor éxito que \textit{Random Dense}. El nivel inferior es ocupado por \textit{Greedy Multi}, cuyo arranque casi nulo limita la expansión del frente y el rendimiento de los algoritmos.

\subparagraph{Análisis por operador de cruce}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_hypervolume_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hipervolumen (Absoluto): Cruce}
...
El análisis descriptivo revela un liderazgo de las configuraciones sustentadas en la inicialización \textit{Greedy Ting}. El primer puesto lo ocupa la configuración NSGA-II con \textit{Greedy Ting} y el cruce uniforme (NS2-GT-UX), alcanzando un valor mínimo de $0{,}0273$. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_igd+_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para IGD+ (Absoluto): Algoritmos}
    \label{fig:dunn_igd+_abs_algorithm}
\end{figure}


La evaluación estadística desde la perspectiva algorítmica estructura los resultados en distintos niveles. En el estrato superior destaca NSGA-II, el cual presenta el mejor rendimiento global sin diferencias estadísticamente significativas frente a AGE-MOEA-II. Este último actúa como puente estadístico, puesto que su desempeño también resulta equivalente al de los MaOEAs basados en vectores de referencia (NSGA-III y U-NSGA-III), los cuales conforman un segundo bloque estadístico.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_igd+_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para IGD+ (Absoluto): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_igd+_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para IGD+ (Absoluto): Cruce}
...
En el segundo nivel se ubica de forma aislada el cruce de dos puntos (2P), seguido en la última posición por el cruce de un punto (1P). Este escalonamiento demuestra que, en cromosomas de alta dimensionalidad como los requeridos para el problema de selección de etiquetas SNP, una alta tasa de recombinación genética resulta fundamental para evitar el estancamiento y alcanzar de forma integral el frente óptimo.
...
Destacan las configuraciones sustentadas en la inicialización \textit{Greedy Multi}. El primer puesto lo ocupa la configuración SMS-EMOA con \textit{Greedy Multi} y el cruce uniforme (SMS-GM-UX), alcanzando un valor mínimo de $0{,}0001$.

Mediante el enfoque \textit{Many-to-One} comparando contra la configuración líder, el bloque de alto rendimiento está compuesto íntegramente por variantes que utilizan \textit{Greedy Multi} . Este resultado es opuesto al observado en IGD+: mientras que \textit{Greedy Multi} maximiza la convergencia puntual al concentrar la población hacia una región muy reducida del espacio objetivo, lo que sitúa los puntos generados muy cerca del frente de referencia, esa misma concentración impide la cobertura global del frente. Esto ilustra una de las tensiones fundamentales entre las métricas de convergencia pura (GD+) y las métricas de convergencia-diversidad conjunta (IGD+ o HV) descritas en la literatura~\cite{ishibuchi2018modified}.

\subparagraph{Análisis por algoritmo}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_gd+_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para GD+ (Absoluto): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_gd+_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para GD+ (Absoluto): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_gd+_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para GD+ (Absoluto): Cruce}
...
El análisis descriptivo revela un dominio de las configuraciones basadas en el algoritmo NSGA-II con la estrategia \textit{Greedy Ting}. La configuración NS2-GT-UX presenta el valor máximo del experimento ($2{,}4229$), seguida de cerca por el resto de variantes NSGA-II con la misma inicialización. 

Mediante el enfoque \textit{Many-to-One} se agrupan estadísticamente las configuraciones; se identifica un primer nivel de significancia compuesto íntegramente por algoritmos que utilizan \textit{Greedy Ting}, confirmando la influencia de la estrategia de inicialización poblacional para la cobertura del espacio de búsqueda .

\subparagraph{Análisis por algoritmo}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_range_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Rango (Absoluto): Algoritmos}
    \label{fig:dunn_range_abs_algorithm}
\end{figure}


La evaluación estadística desde la perspectiva algorítmica revela que NSGA-II obtiene la primera posición absoluta con diferencias estadísticamente significativas frente a todos sus competidores, mostrando la mayor capacidad para mantener una dispersión elevada de las soluciones, atribuida a su operador de distancia de agrupamiento. Por detrás de él, se consolida un grupo de alto rendimiento formado por AGE-MOEA-II, U-NSGA-III, NSGA-III y SPEA2 (sin diferencias significativas entre ellos).

Cabe destacar que SPEA2 actúa como puente estadístico en la comparativa de todos contra todos, ya que también resulta estadísticamente equivalente a SMS-EMOA, situado en un estrato inferior. Finalmente, RVEA ocupa la última posición de forma significativa, registrando los valores más bajos del experimento, lo que refleja una exploración concentrada en una zona limitada del espacio objetivo.

\subparagraph{Análisis por inicialización}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_range_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Rango (Absoluto): Inicialización}
    \label{fig:dunn_range_abs_init}
\end{figure}


La evaluación de las estrategias de inicialización consolida a \textit{Greedy Ting} en el primer nivel de significancia estadística, superando de forma estricta a \textit{Random Dense} y a \textit{Greedy Multi}. Esta superioridad se explica por su efecto topológico: su naturaleza híbrida inyecta soluciones ancla deterministas en la población inicial que aseguran la cobertura temprana de los extremos geométricos del frente de Pareto. Al combinar estos anclajes periféricos con individuos construidos mediante selección voraz y por aleatoriedad, la estrategia garantiza desde la primera generación un despliegue amplio a lo largo del hiperespacio, facilitando la expansión simétrica de los algoritmos hacia los extremos de compacidad y tolerancia.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_minsum_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para MínSuma (Absoluto): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_summin_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para SumaMín (Absoluto): Algoritmos}
    \label{fig:dunn_summin_abs_algorithm}
\end{figure}


La evaluación estadística de los algoritmos establece cuatro niveles con solapamiento. NSGA-II ocupa de forma estricta el primer nivel, con diferencias significativas frente a todos los demás algoritmos. Este resultado se atribuye a su operador de distancia de agrupamiento, que mantiene la presión de selección hacia las soluciones más aisladas del frente, favoreciendo la exploración de la región de mínima suma objetiva.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_summin_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para SumaMín (Absoluto): Inicialización}
    \label{fig:dunn_summin_abs_init}
\end{figure}


La evaluación de las estrategias de inicialización establece tres niveles. \textit{Greedy Ting} ocupa el primer nivel con una media de $0{,}8089$, seguida de \textit{Random Dense} en el segundo nivel ($1{,}0375$). \textit{Greedy Multi} queda relegada al tercer nivel con el valor más elevado ($1{,}8270$).

\subparagraph{Análisis por cruce}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_summin_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para SumaMín (Absoluto): Cruce}
...
Este grupo comprende métricas de dominio específico que evalúan la utilidad práctica de las soluciones de etiquetas SNP encontradas, analizando la tasa de eficiencia media de tolerancia (Tmed) y la diversidad de genotipos seleccionados mediante la distancia media de Hamming (Hmed).

\paragraph{Tasa de Tolerancia Media (Tmed)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avgtolerancerate_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmed (Absoluto): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avgtolerancerate_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmed (Absoluto): Inicialización}
    \label{fig:dunn_avgtolerancerate_abs_init}
\end{figure}


La evaluación de las estrategias de inicialización establece tres niveles. \textit{Random Dense} ocupa el primer nivel con una media de $0{,}2115$, seguida de \textit{Greedy Multi} en el segundo nivel ($0{,}1899$). \textit{Greedy Ting} queda relegada al tercer nivel con el valor más bajo ($0{,}1766$).

El liderazgo de \textit{Random Dense} se explica porque sitúa la población inicial en la región de alta compacidad relativa (en torno al $50\%$ de densidad genotípica), desde donde el EA podría explorar soluciones con tolerancias medias elevadas. \textit{Greedy Ting}, pese a formar parte de la configuración global líder al combinarse con SMS-EMOA y HUX, obtiene el peor rendimiento promedio al obligar al algoritmo a destinar recursos exploratorios a los extremos geométricos del espacio, donde la tolerancia media tiende a decaer.

\subparagraph{Análisis por cruce}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avgtolerancerate_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmed (Absoluto): Cruce}
...
\paragraph{Tasa de Tolerancia Máxima (Tmáx)}
...
El análisis de configuraciones establece a \textit{AGE-MOEA-II + Greedy Multi + UX} como la combinación líder con una tasa máxima media de $0{,}2916$. Sin embargo, la prueba de Dunn agrupa a esta configuración junto con otras 24 combinaciones dentro de la misma clase de equivalencia. Esta extensa agrupación élite (ver Anexo~\ref{tab:elite_maxt_abs}) está dominada por configuraciones que emplean \textit{Greedy Multi}, lo que sugiere que la retención de los SNPs de máxima tolerancia depende fuertemente del punto de partida proporcionado al algoritmo en el espacio de búsqueda.

\subparagraph{Análisis por algoritmo}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_maxtolerancerate_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmáx (Absoluto): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_maxtolerancerate_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmáx (Absoluto): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_maxtolerancerate_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmáx (Absoluto): Cruce}
...
\paragraph{Distancia de Hamming Promedio (Hmed)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avghammingdistance_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hmed (Absoluto): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_nsga2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_spea2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_nsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_unsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_rvea_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_smsemoa_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/1_frentes/frentes_pareto/frentes_pareto_agregados_agemoea2_full_31.png}
...
La mejora más destacable se produce en los MaOEAs (NSGA-III, U-NSGA-III y RVEA); aunque siguen conformando los frentes con menos puntos del estudio ($\sim 42$ soluciones medias para NSGA-III y U-NSGA-III, y $\sim 67$ para RVEA), experimentan un incremento apreciable respecto la escasa cardinalidad de sus frentes en el experimento absoluto ($\sim 26$, $\sim 25$ y $\sim 24$ soluciones respectivamente).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_nsga2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_spea2_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_nsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_unsga3_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_rvea_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_smsemoa_full_31.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/2_metricas_convergencia/convergencia_hv_agemoea2_full_31.png}
...
La observación de estas gráficas confirma que el comportamiento dinámico intergeneracional se mantiene estable a pesar de la reducción del tamaño de la compacidad. Se reproducen de forma visual los mismos fenómenos explicados en profundidad en el experimento anterior, aunque también se ven otros diferentes:

\begin{enumerate}
    \item \textbf{El efecto de impulso temprano de las inicializaciones:} Nuevamente, las estrategias \textit{Random Dense} y \textit{Greedy Ting} inyectan a la población inicial un conjunto de soluciones que influencian positivamente al algoritmo en las primeras generaciones. En contraste, \textit{Greedy Multi} muestra un arranque prácticamente nulo. Este fenómeno se explicó en el experimento anterior.
    \item \textbf{La mejora de RVEA:} A diferencia del decrecimiento documentado en la formulación absoluta con \textit{Greedy Ting} (donde perdía un $40\%$ de calidad métrica), RVEA logra mantener una tendencia ascendente en este experimento. Sus configuraciones con \textit{Greedy Ting} inician con un hipervolumen de $\sim 0{,}254$ y finalizan de forma estable en torno a $\sim 0{,}268$. Este cambio podría deberse a que la propia división fraccional altera el espacio de los objetivos $f_3$ y $f_4$ de tal forma que el algoritmo pueda sostener más soluciones en el frente final, fenómeno que como ya se ha estudiado ocurre con el resto de algoritmos basados en vectores de referencia.
    \item \textbf{El decrecimiento por saturación en SPEA2:} Se documenta un comportamiento particular en este experimento proporcional. El hipervolumen de SPEA2 no experimenta un crecimiento monótono, sino que alcanza un pico de rendimiento a lo largo de su evolución (por ejemplo, $\sim 0{,}350$ en la generación 100 con \textit{Greedy Ting}) para posteriormente comenzar a decrecer de forma paulatina hasta el final de la ejecución (cerrando en $\sim 0{,}331$). Esta involución probablemente tiene lugar debido a un mal desempeño del mecanismo de desempates de SPEA2, que es el principal operador de los EAs en MaOPs, ya que la dominancia de Pareto que caracteriza a este algoritmo pasa a un segundo plano.
\end{enumerate}
...
\setlength{\tabcolsep}{0.5pt}
\begin{longtable}{@{}lccccccccc@{}}
\caption[Resultados de las métricas de evaluación (Proporcional)]{Resultados globales del experimento proporcional (valores promedio y desviación típica en el subíndice a lo largo de 31 ejecuciones independientes). Se resalta en negrita el \textit{Top 5} de configuraciones algorítmicas con mejor rendimiento para cada métrica individual.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_hypervolume_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hipervolumen (Proporcional): Algoritmos}
    \label{fig:dunn_hv_prop_alg}
\end{figure}

Al evaluar los EAs de forma aislada (Figura~\ref{fig:dunn_hv_prop_alg}), el test estadístico permite estructurar el rendimiento en niveles de significancia diferenciados. El primer grupo estadístico está liderado por AGE-MOEA-II, el cual se halla emparejado estadísticamente con NSGA-II. La principal diferencia con respecto al experimento absoluto es que NSGA-II ha perdido el liderazgo estadístico para compartir el primer puesto. 

Bajo esta nueva formulación de objetivos, NSGA-II tarda el doble en eliminar a los individuos dominados (media de 70,2 generaciones), lo que retrasa el momento en el que su distancia de agrupamiento puede tomar el control exclusivo de la dispersión poblacional, aunque esto no indica ser necesariamente un problema para NSGA-II, al contrario, esto permite a NSGA-II explorar de forma más exhaustiva el espacio de soluciones, utilizando su mecanismo de ordenación de frentes.

Un factor estructural fundamental evidenciado empíricamente (Figura~\ref{fig:corr_spearman_prop}) es que la normalización proporcional disminuye significativamente la fuerza general de la correlación entre los objetivos. El análisis de Spearman sobre las soluciones del frente de Pareto final en este experimento ($n = 380\,264$) revela que, mientras en la formulación absoluta todos los objetivos estaban casi perfectamente correlacionados ($|\rho| \geq 0{,}90$, siendo la mayoría $\geq 0{,}96$), en la formulación proporcional la magnitud de la correlación cae de forma generalizada (por ejemplo, a $|\rho| = 0{,}57$ entre tolerancia y disimilitud). Esta menor dependencia incrementa la dimensionalidad efectiva del frente de Pareto. Como establecen Ishibuchi \textit{et al.}~\cite{ishibuchi2015behavior}, el buen rendimiento de NSGA-II en MaOEAs depende de una altísima correlación; al romperse esta redundancia, su distancia de agrupamiento pierde eficacia para cubrir las distintas regiones del frente. Por el contrario, AGE-MOEA-II se adapta orgánicamente a esta mayor complejidad gracias a su estimación geométrica y su capacidad de mantener una reserva de individuos dominados durante las 500 generaciones, como se ha comprobado analizando la evolución de la población.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../exp_aux/correlacion/corr_spearman_experimento_prop.png}
    \caption[Análisis de correlación de Spearman (Proporcional)]{Matriz de correlación de Spearman (izquierda) y p-valores en escala logarítmica (derecha) para los cuatro objetivos en el experimento proporcional. Se evidencia cómo la normalización matemática rompe la alta dependencia lineal observada en el experimento absoluto (véase Figura~\ref{fig:corr_spearman_abs}), introduciendo correlaciones inversas y reduciendo la magnitud general de correlación (e.g., $|\rho| = 0{,}57$).}
    \label{fig:corr_spearman_prop}
\end{figure}

A la luz de estos datos, se puede deducir la razón empírica subyacente por la que Ting \textit{et al.}~\cite{ting2010multi} formularon los objetivos proporcionales originales de esta manera. En un problema biológico donde los objetivos absolutos son intrínsecamente redundantes y sinérgicos, los EAs no se enfrentan a un problema real de optimización de muchos objetivos, ya que el frente de Pareto degenera en una estructura de baja dimensionalidad. Al dividir los objetivos de disimilitud y balance por la compacidad, introdujeron una fuerte descorrelación matemática que transformó el problema en un entorno de muchos objetivos verdaderamente complejo, con conflictos genuinos y alta dimensionalidad efectiva. Si bien es cierto que aún existen correlaciones, son mucho menos acentuadas que en la formulación absoluta. Esta modificación estructural es lo que permite evaluar rigurosamente la capacidad de convergencia y el rendimiento de los algoritmos frente a MaOPs.

El segundo grupo estadístico engloba a los MaOEA (U-NSGA-III y NSGA-III) conjuntamente con SPEA2. Como se ha documentado previamente, los algoritmos basados en vectores de referencia sobre una topología discreta obligan a que parte de los recursos de búsqueda se desvíen hacia regiones infactibles, reduciendo la cardinalidad del frente poblacional resultante~\cite{ishibuchi2015behavior}. SPEA2 (cuya saturación de dominancia poblacional promedia unas estables 46,0 generaciones) iguala el rendimiento de este grupo debido a su mecanismo de archivo externo y a su estimador basado en el $k$-ésimo vecino más cercano ($k$-NN).

En un estrato inferior se ubica SMS-EMOA junto con SPEA2, que actúa como puente estadístico. El escaso rendimiento de SMS-EMOA agrava los problemas detectados en el experimento absoluto: su arquitectura de estado estacionario provoca una convergencia excesiva en la que el 100\% de la población se concentra en el primer frente en una media de 5,2 generaciones. Al eliminar cualquier traza de diversidad genética prácticamente tras el inicio de la ejecución, es incapaz de lograr la expansión del frente.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_hypervolume_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hipervolumen (Proporcional): Inicialización}
...
Con respecto al experimento absoluto, \textit{Random Dense} ya no es la inicialización ganadora, sino \textit{Greedy Ting}. Bajo esta nueva formulación, los paneles grandes de SNPs no se ven tan recompensados como antes, lo que hace que las soluciones iniciales aleatorias sean menos competitivas. Las soluciones iniciales de \textit{Greedy Ting}, por el contrario, se ven beneficiadas por la nueva formulación, ya que están diseñadas para optimizar los objetivos planteados, concretamente los proporcionales.

\subparagraph{Análisis por operador de cruce}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_hypervolume_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hipervolumen (Proporcional): Cruce}
    \label{fig:dunn_hv_prop_cross}
\end{figure}

El estudio de los operadores de cruce (Figura~\ref{fig:dunn_hv_prop_cross}) evidencia un mayor desempeño de los operadores uniformes, como se lleva observando a lo largo del trabajo. Las configuraciones HUX y UX conforman un grupo estadístico superior, siendo estadísticamente equivalentes entre sí y mostrando mejoras sobre los cruces por n-puntos (2P y 1P).
...
El test estadístico de Dunn-Holm agrupa un conjunto de élite conformado por 26 configuraciones (listadas en el Anexo~\ref{tab:elite_igd_prop}). En este bloque de mayor rendimiento, existe un dominio de la inicialización \textit{Greedy Ting}, la cual está presente en la práctica totalidad de las mejores combinaciones.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_igd+_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para IGD+ (Proporcional): Algoritmos}
...
Es apreciable que los resultados de esta métrica difieren con los ofrecidos por la misma en el experimento absoluto. En este experimento, los grupos estadísticos son más grandes, lo que podría deberse a una reducción del tamaño de los diferencias en esta métrica durante este experimento.

\subparagraph{Análisis por inicialización}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_igd+_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para IGD+ (Proporcional): Inicialización}
    \label{fig:dunn_igd_prop_init}
\end{figure}

El estudio de los métodos de generación de la población inicial (Figura~\ref{fig:dunn_igd_prop_init}) confirma el liderazgo de \textit{Greedy Ting}, la cual se posiciona en el primer nivel estadístico de manera estricta. Le sigue \textit{Random Dense} y, en último lugar, \textit{Greedy Multi}.

Son los mismos ranquines que en el experimento absoluto.

\subparagraph{Análisis por operador de cruce}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_igd+_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para IGD+ (Proporcional): Cruce}
...
Se obtuvo el mismo ranquin en el experimento absoluto.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_gd+_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para GD+ (Proporcional): Inicialización}
    \label{fig:dunn_gd_prop_init}
\end{figure}

El estudio de las estrategias de generación inicial (Figura~\ref{fig:dunn_gd_prop_init}) ratifica el liderazgo de \textit{Greedy Multi}, que se establece en el primer escalón estadístico, seguida por \textit{Random Dense} y relegando a \textit{Greedy Ting} a la última posición.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_gd+_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para GD+ (Proporcional): Cruce}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_range_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Rango (Proporcional): Algoritmos}
    \label{fig:dunn_range_prop_alg}
\end{figure}

El estudio del rendimiento aislado de los EAs (Figura~\ref{fig:dunn_range_prop_alg}) sitúa a NSGA-II como el líder en el primer nivel de significancia estadística. Por debajo de él se conforma un segundo bloque equivalente en el que conviven AGE-MOEA-II, U-NSGA-III, NSGA-III y SPEA2, sin que existan diferencias estadísticas significativas entre el primero y el último de este grupo. En la última posición se encuentra RVEA, el cual, debido a su mecanismo basado en vectores de referencia, experimenta mayores dificultades para mantener una alta diversidad extensional.

El ránking es prácticamente el mismo que el obtenido en el experimento absoluto.

\subparagraph{Análisis por inicialización}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_range_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Rango (Proporcional): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_range_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Rango (Proporcional): Cruce}
    \label{fig:dunn_range_prop_cross}
\end{figure}

El comportamiento de los operadores de cruce (Figura~\ref{fig:dunn_range_prop_cross}) sigue un patrón consistente en la resolución del problema de selección de etiquetas SNP. Los cruces con una alta tasa de intercambio genético (HUX y UX) superan significativamente al resto, integrando el primer bloque estadístico de equivalencia. Por el contrario, los cruces de dos y un punto (2P y 1P) forman el estrato inferior, confirmando que la recombinación uniforme favorece la exploración y la extensión máxima del conjunto de soluciones no dominadas.

El ránking es bastante similar al obtenido en el experimento absoluto.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_minsum_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para MínSuma (Proporcional): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_minsum_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para MínSuma (Proporcional): Inicialización}
    \label{fig:dunn_minsum_prop_init}
\end{figure}

La evaluación de las estrategias de inicialización establece tres niveles sin solapamiento. \textit{Random Dense} ocupa el primer nivel ($\mu = 1{,}4340$). \textit{Greedy Multi} se sitúa en el segundo ($\mu = 1{,}5156$) y \textit{Greedy Ting} en el tercero ($\mu = 1{,}5164$).

Este resultado es consistente con el experimento absoluto, donde \textit{Random Dense} también lideró MínSuma. Sin embargo, como se identificó en el análisis por configuración, \textit{Greedy Multi} está un puesto por encima de \textit{Greedy Ting}, cambiando el puesto que tenía en el experimento previo.

\subparagraph{Análisis por cruce}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_minsum_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para MínSuma (Proporcional): Cruce}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_summin_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para SumaMín (Proporcional): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_summin_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para SumaMín (Proporcional): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_summin_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para SumaMín (Proporcional): Cruce}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avgtolerancerate_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmed (Proporcional): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avgtolerancerate_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmed (Proporcional): Inicialización}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_maxtolerancerate_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmáx (Proporcional): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_maxtolerancerate_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmáx (Proporcional): Inicialización}
    \label{fig:dunn_maxt_prop_init}
\end{figure}

La evaluación de las estrategias de inicialización establece tres niveles. \textit{Greedy Multi} lidera en solitario ($\mu = 0{,}2663$), seguida de \textit{Random Dense} en el segundo nivel ($\mu = 0{,}2525$) y \textit{Greedy Ting} en el tercero ($\mu = 0{,}2370$).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_maxtolerancerate_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Tmáx (Proporcional): Cruce}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avghammingdistance_algorithm_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hmed (Proporcional): Algoritmos}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avghammingdistance_init_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hmed (Proporcional): Inicialización}
    \label{fig:dunn_avghd_prop_init}
\end{figure}

La evaluación de las estrategias de inicialización establece tres niveles. \textit{Random Dense} domina el primer nivel con una media de $103{,}04$, seguida de \textit{Greedy Ting} en el segundo ($43{,}13$) y \textit{Greedy Multi} en el tercero ($10{,}17$).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../exp/full_31/PROP_20260624T140801/2_comparativa/4_rankings/diagramas_equivalencia/equivalencia_dunn_avghammingdistance_crossover_full_31.png}
    \caption{Ranquin y Equivalencia Estadística para Hmed (Proporcional): Cruce}
...
\section{Análisis de influencia paramétrica (Tamaño del efecto)}
\label{sec:tamano_efecto}

Tras evaluar de forma aislada las distribuciones y los ranquines estadísticos de los diferentes algoritmos, inicializaciones y operadores a lo largo de todas las métricas, resulta fundamental determinar matemáticamente el peso real que ejerce cada uno de estos factores sobre el rendimiento final de una configuración. 

Para cuantificar esta influencia, tal y como se estableció en la metodología, se ha calculado el tamaño del efecto (Eta-cuadrado de Kruskal-Wallis, $\eta^2_H$) iterando sobre las $2.604$ réplicas independientes de todas las métricas en ambos experimentos (absoluto y proporcional). Al promediar los resultados, se obtienen los siguientes porcentajes globales de varianza explicada por cada factor:

\begin{itemize}
    \item \textbf{Inicialización poblacional:} $64{,}46\%$
    \item \textbf{Algoritmo evolutivo:} $16{,}67\%$
    \item \textbf{Operador de cruce:} $4{,}51\%$
\end{itemize}

Estos datos demuestran que la estrategia de inicialización es, con diferencia, el parámetro rector en la resolución del problema de selección de etiquetas SNP mediante EAs, explicando por sí sola casi dos tercios de la variabilidad del rendimiento final. El algoritmo actúa como un optimizador secundario, por detrás de la estrategia de inicialización poblacional, mientras que el impacto del operador de cruce es bastante menor (inferior al $5\%$). Este hallazgo resulta coherente con el teorema del \textit{No Free Lunch} \cite{wolpert1997no}, demostrando que la inyección de conocimiento del dominio en la inicialización es más relevante que la elección del motor de búsqueda.

\section{Contraste con el estudio de referencia}
\label{sec:contraste_moqa}

Esta sección contrasta de forma cualitativa las conclusiones del estudio de referencia, Moqa \textit{et al.} \cite{moqa2022assessing}, con los resultados obtenidos en el experimento proporcional. La comparativa se restringe a las métricas comunes a ambos estudios (Rango, SumaMín, MínSuma, Tmáx, Tmed y Hmed) dado que Moqa \textit{et al.} no emplean Hipervolumen, IGD+ ni GD+.

Antes de proceder, se deben señalar las diferencias metodológicas entre ambos estudios que limitan la comparabilidad directa de los valores absolutos. En primer lugar, la transformación de los objetivos de maximización difiere: en este estudio se emplea la negación lineal ($f' = -f$), mientras que Moqa \textit{et al.} no especifican explícitamente el método, aunque se hipotetiza, a partir de su estudio de referencia Ting \textit{et al.} \cite{ting2010multi}, que aplican una inversión no lineal ($f' = 1/f$).

Esta diferencia distorsiona la topología del espacio de búsqueda y hace que las magnitudes numéricas no sean directamente comparables. En segundo lugar, el tamaño de la población difiere (220 individuos en este estudio frente a 200 en Moqa \textit{et al.}). Además, en este TFG se ejecutan 31 réplicas independientes por configuración, mientras en que Moqa \textit{et al.} se ejecutan 5. Por consiguiente, la comparativa se centra exclusivamente en tendencias ordinales y en el análisis de las conclusiones cualitativas del estudio original.

\subsubsection*{Liderazgo de MOEA/D}

Moqa \textit{et al.} concluyen que MOEA/D supera al resto de algoritmos en la mayoría de casos. Esta afirmación no puede contrastarse en este trabajo, ya que MOEA/D fue excluido del diseño experimental por incompatibilidad con la gestión nativa de la restricción de factibilidad biológica del problema en la librería \texttt{pymoo}.

\subsubsection*{Superioridad de NSGA-III en tolerancia máxima}

Los autores afirman que NSGA-III supera a NSGA-II y SPEA2 en la métrica de tolerancia máxima (Tmáx). En el experimento proporcional de este estudio, NSGA-III es estadísticamente igual que SPEA2, si bien es cierto que es superior. Por otra parte, NSGA-II se encuentra por debajo de este nivel de significancia. Por lo tanto, la superioridad exclusiva de NSGA-III para esta métrica se reproduce, pero solo parcialmente.

\subsubsection*{Liderazgo de SPEA2 en distancia media de Hamming}

Moqa \textit{et al.} afirman que SPEA2 supera a todos los algoritmos en la métrica de distancia media de Hamming (Hmed). Esta conclusión queda directamente refutada en el experimento proporcional: AGE-MOEA-II ocupa el primer nivel estadístico en solitario, mientras que SPEA2 desciende al último bloque estadístico.

\subsubsection*{Ventaja de la inicialización \textit{Greedy}}

Los autores concluyen que la inicialización heurística \textit{Greedy} mejora los resultados frente a la aleatoria para NSGA-III, SPEA2 y MOEA/D. Esta tendencia se valida de forma parcial en este estudio: \textit{Greedy Ting} lidera las métricas de extensión del frente (Rango, SumaMín), mientras que \textit{Random Dense} domina las métricas de tolerancia (Tmed) y de diversidad (Hmed). La superioridad de la inicialización heurística no es universal, sino dependiente de la métrica de rendimiento que se priorice.

\subsubsection*{Factor de rendimiento predominante}

En los ranquines de Moqa \textit{et al.}, el factor predominante observable es el algoritmo: en la mayoría de métricas, las dos configuraciones del mismo algoritmo (con inicialización aleatoria y heurística) aparecen contiguas en los primeros puestos, lo que indica que el buscador evolutivo determina el nivel de rendimiento y la inicialización lo modula de forma secundaria.

En este estudio se observa el patrón contrario. El análisis de los ranquines de configuraciones muestra que la inicialización es el factor que estructura verticalmente el rendimiento: las posiciones más altas están ocupadas por configuraciones con \textit{Greedy Ting} o \textit{Random Dense}, con independencia del algoritmo empleado, mientras que las configuraciones con \textit{Greedy Multi} quedan relegadas. La elección del algoritmo produce una diferenciación de rendimiento visible entre los candidatos de cada bloque de inicialización, pero no determina por sí sola la posición global en el ranquin de configuraciones.

\subsubsection*{Limitaciones de la comparativa}

En conclusión, las divergencias identificadas entre los resultados de este trabajo y los reportados por Moqa \textit{et al.} ponen de manifiesto la sensibilidad del problema a la parametrización, el diseño algorítmico y los métodos de normalización de objetivos. Sin embargo, la imposibilidad de acceder al código fuente original del estudio de referencia impide trazar el origen exacto de estas discrepancias. Las diferencias podrían radicar en detalles de implementación no documentados tales como la lógica interna de la inicialización voraz, los criterios de reparación de individuos infactibles, o incluso errores formales en el registro de los datos. En consecuencia, la validación definitiva de las conclusiones de Moqa \textit{et al.} requiere de la disponibilidad de su software para realizar una evaluación bajo condiciones experimentales idénticas.
