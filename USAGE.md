...
Opciones:
- `--mode` / `-m`: modo de ejecución (`fast`, `medium`, `high`, `full`, `full_20`, `full_30`, `post_processing`).
...
- `frentes_pareto_<modo>.csv` (necesario para regenerar los gráficos del frente de Pareto en modo post_processing)

Comportamiento del modo `post_processing` para los gráficos de Pareto:
- Si existe `frentes_pareto_*.csv` y tiene el esquema esperado, se generan los gráficos de Pareto utilizando los datos del directorio `snp_tag/input/`.
