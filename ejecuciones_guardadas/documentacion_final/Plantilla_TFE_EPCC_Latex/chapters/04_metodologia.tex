...
Para dar contexto a los costes computacionales asociados a los algoritmos, se detalla el entorno utilizado para las ejecuciones del proyecto.
...
En cuanto al software, el proyecto se ha desarrollado íntegramente en Python \cite{python} (versión 3.13.9). Para la ejecución de los EAs se ha utilizado \texttt{pymoo} \cite{pymoo}, un marco de optimización de código abierto. La elección de \texttt{pymoo} como marco principal de desarrollo se justifica por su orientación específica hacia la investigación en MOEAs y MaOEAs, y su arquitectura orientada a objetos. Esta característica ha sido fundamental para el desarrollo del proyecto, ya que ha permitido implementar estrategias de inicialización poblacional como las técnicas voraces y modelar el problema de la selección de etiquetas SNP extendiendo las clases base de la librería.
...
Por otra parte, un etiqueta SNP se define como un polimorfismo representativo (es decir, una variación específica en la secuencia de ADN) que, al actuar en conjunto con otros etiquetas SNP, permite capturar la variación genética de un bloque de haplotipos sin necesidad de secuenciarlos en su totalidad \cite{bafna2003haplotypes, ting2010multi}. Al seleccionar estos SNPs representativos, se logra reducir la dimensionalidad de los datos originales. 
...
Aplicando este mismo principio de filtrado al bloque completo de 1032 SNPs de Hinds \textit{et al.} \cite{hinds2005whole}, se logró eliminar el ruido y reducir la cadena a un total de $n = 772$ SNPs polimórficos efectivos. Este tamaño de entrada determina la longitud de los individuos con los que trabajarán los algoritmos del estudio.
...
Esta formulación permite definir la magnitud del espacio de búsqueda ($\Omega$). Para el conjunto filtrado, la dimensión del espacio combinatorio sobre el que operarán los algoritmos es:
\[ 
|\Omega| = 2^{772} 
\]
...
Para realizar un estudio comparativo exhaustivo, se ha seleccionado un conjunto diverso de EAs. Dado que el problema de selección de etiquetas SNP presenta una dimensionalidad de cuatro objetivos (catalogándose formalmente como un MaOP), la comparativa incorpora tanto MOEAs como MaOEAs. Concretamente, se han incluido todos los algoritmos evaluados en el estudio de referencia de Moqa \textit{et al.} \cite{moqa2022assessing}, ampliando la lista con otros adicionales para abarcar un espectro algorítmico más amplio. Esta selección final cubre diferentes paradigmas evolutivos, lo que permite analizar cómo operan sus distintas estrategias de búsqueda empleando ambas formulaciones de objetivos. Los algoritmos elegidos se agrupan en las siguientes categorías:

\subsection{Algoritmos basados en dominancia de Pareto}
Este paradigma constituye el enfoque clásico, donde la presión de selección recae principalmente sobre la relación de dominancia de Pareto entre las soluciones de la población. Teóricamente, como demostró la revisión de Ishibuchi \textit{et al.} \cite{ishibuchi2008evolutionary}, cuando el problema escala a cuatro o más objetivos, casi todos los individuos de la población pasan a ser no dominados. Esto anula la presión selectiva de la relación de Pareto y obliga al algoritmo a depender de sus métricas de diversidad (como el distancia de agrupamiento de NSGA-II). 

A pesar de esta conocida limitación teórica, se incluyen algoritmos clásicos como NSGA-II o SPEA2 con la intención de reproducir el estudio de referencia \cite{moqa2022assessing}.
\begin{itemize}
    \item \textbf{NSGA-II:} El algoritmo \textit{Non-dominated Sorting Genetic Algorithm II} \cite{deb2002fast} clasifica a los individuos en frentes de no dominancia y aplica un operador de distancia de agrupamiento para preservar la diversidad poblacional.
...
\begin{itemize}
    ...
    \item \textbf{AGE-MOEA-II:} El \textit{Approximation-Guided Evolutionary Algorithm II} \cite{panichella2022adaptive} sustituye el hipervolumen por una puntuación de supervivencia basada en aproximación geométrica adaptativa de la topología del frente de Pareto. Ejerce una presión selectiva sumamente competitiva, reduciendo el coste computacional.
\end{itemize}
...
\Require $M$: Tamaño de la población
\Require $LimitesBio$: Tolerancia máxima posible del conjunto de datos
\Ensure Población inicial $P$
...
Los hiperparámetros globales compartidos por todos los algoritmos se han fijado de acuerdo con los valores habitualmente empleados en la literatura de MOEAs y de MaOEAs, y se detallan a continuación:
...
\begin{itemize}
    ...
    \item \textbf{Probabilidad de mutación ($p_m$):} $1/n$, donde $n = 772$ es el número de SNPs polimórficos efectivos del conjunto de datos; de este modo se espera que se produzca, de media, una mutación por cromosoma.
    ...
    \begin{enumerate}
        \item \textbf{Absoluta:} Objetivos evaluados en su escala natural, empleando límites fijos y estáticos derivados  del conjunto de datos para la normalización.
...
En resumen, el uso de estos límites estáticos extraídos del conjunto de datos es indispensable para evaluar rigurosamente el rendimiento mediante métricas estadísticas en un estudio que consta de 2 experimentos (uno mediante formulación de objetivos absoluta y otro proporcional). Si la normalización se realizase basándose en los resultados de cada experimento (extrayendo los valores máximos y mínimos descubiertos por toda la batería de algoritmos de un experimento), el tamaño del espacio matemático variaría en cada uno de ellos.

Las fluctuaciones estocásticas en las soluciones encontradas desplazarían las fronteras geométricas, lo que invalidaría por completo la comparación de métricas (como el hipervolumen) entre ambos. Al definir vectores de referencia teóricos e inmutables, extraídos del conjunto de datos, se asegura que el espacio normalizado sea siempre idéntico, garantizando una comparativa analítica justa bajo las mismas condiciones espaciales.
...
\begin{itemize}
    \item \textbf{Generational Distance Plus (GD+) (a minimizar):} Como complemento al indicador anterior, la métrica \textbf{GD+} mide la distancia analítica \textit{desde} las soluciones descubiertas por el algoritmo \textit{hacia} el frente de referencia empírico. Actúa como un indicador de convergencia estricta: penaliza si alguna solución descubierta es de mala calidad y se ubica lejos del frente de referencia empírico, sin tener en cuenta la dispersión de las mismas.
...
\end{figure}
    ...
    \item \textbf{Rango (a maximizar):} Suma de los rangos por objetivo del frente normalizado; valora la diversidad geométrica global, entendiéndola por la expansión del frente de Pareto en los ejes extremos. Para ilustrar este concepto analítica y visualmente (véase la Figura \ref{fig:metric_range}), se representa un escenario bidimensional. El frente de alta diversidad (círculos azules) distribuye sus soluciones abarcando desde la coordenada $1{,}0$ hasta la $6{,}0$ en ambos ejes. Esto resulta en una amplitud de $5{,}0$ unidades por objetivo, sumando un Rango global de $10{,}0$. En contraposición, el frente de baja diversidad (cuadrados rojos) presenta soluciones concentradas en una región estrecha, abarcando únicamente desde $3{,}5$ hasta $4{,}2$ en el eje $f_1$, y desde $3{,}8$ hasta $4{,}5$ en el eje $f_2$. Al concentrarse sus proyecciones hacia rangos de tan solo $0{,}7$ unidades por objetivo, su métrica final cae a un valor escalar de $1{,}4$, evidenciando matemáticamente la escasez de variedad en las opciones descubiertas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../figuras_metricas/range_metric.pdf}
    \caption[Representación geométrica de la métrica Rango]{Representación geométrica bidimensional de la métrica Rango. Se contrasta analíticamente la proyección espacial de un frente con alta amplitud (círculos azules) frente a una agrupación de soluciones con baja dispersión (cuadrados rojos).}
...
\begin{itemize}
    \item \textbf{Mínimo de las Sumas (MínSuma) (a minimizar):} Valor mínimo de la suma de objetivos del frente; evalúa qué configuración es capaz de encontrar la solución más equilibrada que contenta a todos los objetivos al mismo tiempo.

    \item \textbf{Suma de los Mínimos (SumaMín) (a minimizar):} Suma de los mejores valores alcanzados en cada objetivo; evalúa qué configuración ha encontrado las soluciones más extremas en cada eje de manera marginal.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc|c}
        ...
        \hline
    \end{tabular}
    \caption[Evaluación analítica de MínSuma y SumaMín]{Ejemplo analítico para la evaluación de SumaMín y MínSuma. El cálculo de MínSuma requiere identificar la única solución con mejor balance general, lo que equivale a seleccionar el valor mínimo de la última columna ($1{,}6$, correspondiente a $S_2$). El cálculo de SumaMín requiere la extracción de los mínimos marginales (resaltados en negrita en cada columna de los objetivos), resultando en la suma $0{,}1 + 0{,}2 + 0{,}1 + 0{,}3 = 0{,}7$.}
...
Dado el carácter estocástico de los EAs, los resultados de las 31 réplicas independientes \cite{granado2020multiobjective} de cada configuración se someten a un análisis estadístico inferencial no paramétrico para validar las diferencias de rendimiento. En todas las pruebas inferenciales de este estudio se establece un nivel de significancia estricto de $\alpha = 0{,}05$.

\subsubsection{Pruebas de significancia}

Para evaluar el desempeño en cada uno de los indicadores aislados, se aplica la prueba ómnibus de Kruskal-Wallis \cite{kruskal1952use}. En caso de detectarse diferencias significativas globales, se emplea el test \textit{post-hoc} de Dunn \cite{dunn1964multiple} para identificar la significancia estadística entre pares específicos de configuraciones.

\subsubsection{Grupos de equivalencia}

Con el fin de facilitar la interpretación visual tanto de los resultados de los ránquines como de los resultados del test de Dunn, se emplean gráficas de ranquin y equivalencia estadística. En estos diagramas (ilustrados mediante el ejemplo adelantado de la Figura \ref{fig:ejemplo_ranquin}), los elementos evaluados (ya sean algoritmos, estrategias de inicialización u operadores de cruce) se representan mediante nodos rojos y se ordenan en el eje vertical según su rendimiento medio, posicionando las mejores opciones en la parte superior y las peores en la parte inferior. 

La característica fundamental de estos diagramas son las líneas de conexión que agrupan a diferentes elementos. Cuando una línea continua enlaza a un conjunto de elementos, indica que \textbf{no existe una diferencia estadística significativa} entre ellos según el nivel $\alpha$ estipulado; es decir, conforman un \textit{grupo de equivalencia}. Por el contrario, la ausencia de una línea compartida entre dos elementos demuestra que uno supera significativamente al otro. En aquellos casos donde un elemento pertenece simultáneamente a varias líneas superpuestas, significa que actúa como puente estadístico entre distintos grupos.
...
\begin{figure}[H]
    ...
    \caption[Interpretación de gráfica de ranquin y equivalencia estadística]{Ejemplo ilustrativo de una gráfica de ranquin y equivalencia estadística real extraída de los resultados.}
    \label{fig:ejemplo_ranquin}
\end{figure}

\subsubsection{Matrices de significancia estadística}

De forma complementaria a las gráficas de ranquin, en el Anexo \ref{anexo:significancia_abs} se emplean matrices de significancia estadística. Estas representaciones bidimensionales permiten visualizar exhaustivamente el resultado cruzado del test \textit{post-hoc} de Dunn para todas las combinaciones posibles de elementos evaluados.

Para interpretar correctamente estas matrices, deben cruzarse las filas y columnas correspondientes a los dos elementos que se desean comparar. La intensidad y tonalidad de las celdas dictaminan el nivel de significancia del contraste de hipótesis:

\begin{itemize}
    \item \textbf{Verde oscuro (\textit{Alta Sig.}):} Indica de forma empírica una diferencia estadística altamente significativa entre ambos elementos ($p < 0{,}01$).
    \item \textbf{Verde claro (\textit{Sig.}):} Denota una diferencia estadísticamente significativa estándar ($0{,}01 \le p < 0{,}05$).
    \item \textbf{Rosa pálido (\textit{No Sig.}):} Concluye que no existe evidencia estadística suficiente ($p \ge 0{,}05$) para afirmar que un elemento sea superior al otro, declarándose como estadísticamente equivalentes.
\end{itemize}

La Figura \ref{fig:ejemplo_heatmap} ilustra un ejemplo didáctico de estas matrices aplicado a la comparación aislada de algoritmos.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../exp/full_31/ABS_20260625T071958/2_comparativa/4_rankings/heatmap_dunn_igd+_algorithm_full_31.png}
    \caption[Interpretación de matriz de significancia estadística]{Ejemplo ilustrativo de una matriz de significancia estadística entre algoritmos. Las celdas en tonos verdes denotan diferencias significativas ($p < 0{,}05$) entre el algoritmo de la fila y el de la columna correspondientes, segmentadas por intensidad según su grado de certidumbre. Las celdas de color rosa representan una equivalencia estadística empírica entre ellos.}
    \label{fig:ejemplo_heatmap}
\end{figure}

\subsubsection{Tamaño del efecto}

Adicionalmente, para determinar la magnitud de la influencia de cada factor de variación (algoritmo, inicialización y cruce) sobre el rendimiento final, el análisis de Kruskal-Wallis se complementó con el cálculo del tamaño del efecto mediante el índice Eta-cuadrado ($\eta^2_H$) \cite{tomczak2014need}. Esta métrica, acotada entre $0$ y $1$, permite cuantificar matemáticamente qué porcentaje de la varianza total observada en los resultados es atribuible de forma aislada a cada uno de los parámetros de configuración.

\subsection{Representación de frentes de Pareto}

Para el análisis visual de los frentes de Pareto obtenidos por cada EA, se emplea un proceso de agregación de resultados. Debido al carácter estocástico de estas técnicas, la representación gráfica se construye uniendo la réplica con el valor mediano en la métrica de hipervolumen \cite{granado2020multiobjective} de todas las configuraciones evaluadas para un mismo algoritmo (tres estrategias de inicialización poblacional y cuatro operadores de cruce), resultando de este modo en un total de 8 gráficas.
...
\subsection{Análisis de la dinámica de convergencia}

Para analizar cómo evoluciona la convergencia de la población a lo largo de las 500 generaciones estipuladas por el diseño experimental, se estudia gráficamente la trayectoria generacional de la métrica de Hipervolumen. El hipervolumen se consolida como el indicador idóneo para analizar el progreso de la búsqueda debido a su naturaleza integral: al penalizar simultáneamente tanto la lejanía respecto al frente óptimo (falta de convergencia) como la concentración de soluciones (falta de diversidad), el crecimiento continuo de esta métrica es una garantía matemática de que el algoritmo está descubriendo mejores compromisos biológicos.

Para garantizar que las curvas de convergencia resultantes constituyan una representación fidedigna del comportamiento algorítmico típico, mitigando los sesgos del ruido estocástico intrínseco a los EAs, se ha aplicado el mismo proceso de selección representativa: de las 31 ejecuciones independientes generadas por configuración, se selecciona y se traza exclusivamente la trayectoria de la réplica con el valor mediano en la métrica de hipervolumen final. En la Figura \ref{fig:conv_hv_metodologia} se ilustra el resultado visual de este proceso.
...
\begin{figure}[H]
    ...
    \caption[Representación de la dinámica de convergencia]{Ejemplo ilustrativo de la representación gráfica empleada para analizar la dinámica de convergencia. El eje de abscisas detalla la evolución temporal (generaciones), mientras que el eje de ordenadas cuantifica el crecimiento escalar del hipervolumen (HV).}
    \label{fig:conv_hv_metodologia}
\end{figure}
