...
> Este repositorio contiene la implementación y documentación del **Trabajo de Fin de Grado**:
> **"Resolución del problema del etiquetado de SNPs usando computación evolutiva"**
> - **Autor:** Samuel Corrionero Fernández
> - **Tutor:** Dr. José María Granado Criado
> - **Titulación:** Grado en Ingeniería Informática en Ingeniería de Computadores
> - **Institución:** Escuela Politécnica, Universidad de Extremadura (UEx), Cáceres, España
...
8. **[Autoría y Contexto Académico](#autoría-y-contexto-académico)**
9. **[Licencia](#licencia)**
...
*Figura 6: Mapa de calor de la estructura haplotípica sintética, donde se observa la coherencia structural inducida por el modelo de cadenas LD.*
...
* **Estrategias de Inicialización**:
  ...
  * `greedy_holistic`: La estrategia más avanzada del sistema. Utiliza un enfoque de **5 niveles (Tiers)**:
    ...
    3. **Ensamblaje por Bloques LD**: Diversidad structural basada en ligamiento real.
...
### Interfaz de Línea de Comandos (CLI) y Uso

El sistema se ejecuta como un paquete modular desde la raíz del repositorio a través del punto de entrada unificado:
...
* `--mode` / `-m`: Define el modo de ejecución (`fast`, `medium`, `high`, `full`, `full_20`, `full_30`, `post_processing`).
* `--data-source` / `-d`: Especifica la fuente de datos (`hinds2005` o `synthetic`).

#### Ficheros CSV Generados

En ejecuciones normales, los resultados se almacenan dentro del subdirectorio `.../1_ejecuciones/`:
* `resultados_detallados_<modo>.csv`: Registro exhaustivo con todas las métricas de rendimiento por réplica y configuración.
* `historico_generacional_<modo>.csv`: Evolución generacional del algoritmo para análisis de convergencia.
* `frentes_pareto_<modo>.csv`: Conjunto de soluciones no dominadas (necesario para regenerar los gráficos del frente de Pareto en modo `post_processing`).

#### Comportamiento del Modo `post_processing`

El modo `post_processing` permite analizar y regenerar reportes visuales de ejecuciones previas sin volver a ejecutar la búsqueda evolutiva:
* **Generación de gráficos de Pareto**: Si existe `frentes_pareto_*.csv` y cumple con el esquema esperado, se reconstruyen las gráficas del frente de Pareto utilizando los datos almacenados en `snp_tag/input/`.
* **Resiliencia ante datos ausentes**: Si el fichero no está disponible o le faltan columnas requeridas, la sección de Pareto se omite mostrando un aviso explícito en la consola, continuando con la generación del resto de reportes estadísticos y comparativos.

#### Configuración vía `user_config.ini`

Los parámetros tunables de la simulación y del motor evolutivo se gestionan en el archivo `user_config.ini`.
Formato recomendado:
```ini
[Seccion]
clave = valor ; explicación
...
#### Dashboard Dinámico e Hipervínculos CLI
El sistema ofrece un dashboard dinámico en terminal que reporta el progreso en tiempo real. Además, utiliza secuencias **OSC 8** para generar hipervínculos clicables directamente en la terminal, permitiendo abrir los reportes CSV y figuras PDF de forma instantánea al finalizar el experimento.
...
### Gestiones de Escalas y Normalización
...
Este repositorio y el código fuente forman parte del **Trabajo de Fin de Grado (TFG)** titulado **"Resolución del problema del etiquetado de SNPs usando computación evolutiva"** para la obtención del título de **Grado en Ingeniería Informática en Ingeniería de Computadores**.

* **Autor:** Samuel Corrionero Fernández
* **Tutor:** Dr. José María Granado Criado
* **Titulación:** Grado en Ingeniería Informática en Ingeniería de Computadores
* **Centro:** Escuela Politécnica
* **Institución:** Universidad de Extremadura (UEx), Cáceres, España

Todo el código está abierto a revisión académica y constituye un esfuerzo por proporcionar una herramienta escalable, modular y rigurosa para la comunidad de bioinformática e investigación en algoritmos evolutivos.

---

## Licencia

Este proyecto se distribuye bajo la [Licencia MIT](LICENSE). Eres libre de utilizar, modificar y distribuir este código con fines académicos o de investigación, siempre y cuando se incluya una copia de la licencia y se cite la autoría original.
